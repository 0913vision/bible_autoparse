...
def cutRange(bible):
    ...
    return begin, end, bible[begin-1:end]

def WriteOnCB(book_name, chap, begin, end, target):
    if begin<end:
        clipboard.copy(book_name + "\r\n" + str(chap) + ":" + str(begin) + "~" + str(end) + "\n\n")
    else:
        clipboard.copy(book_name + "\r\n" + str(chap) + ":" + str(begin) + "\n\n")
...
a, b, target = cutRange(bible)

WriteOnCB(book_name, chap, a,b,target)
